...
# 创建卫星星系
def Creat_satellite(numOrbitPlanes=5, numSatsPerPlane=3, hight=550, Inclination=53, name='Sat'):
...
def Creating_All_Access():
    ...
    for each_sat in tqdm(sat_list):
        ...
        access_left = now_sat_transmitter.GetAccessToObject(
            Get_sat_receiver(sat_dic['Sat' + str((now_plane_num - 1) % 5) + '_' + str(now_sat_num)]))
        # 与右面的卫星的接收机相连
...
# 如果不是读取当前场景，即首次创建场景
if not Read_Scenario:
    Creat_satellite(numOrbitPlanes=5, numSatsPerPlane=5, hight=550, Inclination=53)
