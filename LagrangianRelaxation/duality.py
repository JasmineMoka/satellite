...
class Duality:
    ...
    def update(self):
        self.m += self.step_size * self.subgrad
        return self.m
    ...
    def print_status(self, k, max_iter):
        flag = max_iter // 20
        if k % flag == 0 or k == max_iter - 1:
            print("Iteration {:d}/{:d} - Obj value: {:0.2f}, Step size: {:0.4f}, Gap: {:0.4f}%".format(
                k + 1, max_iter, value(self.relaxed_model.obj), self.step_size, self.gap))
    def plot_trace(self):
        plt.figure()
        plt.subplot(2,2,1)
