...
# model.energy_local = Param(model.M, model.N, within=NonNegativeReals, initialize=1)
#
# model.energy_bs = Param(model.M, model.N, within=NonNegativeReals, initialize=1)
#
...
# 定义变量
# model.x = Var(model.M, model.N, within=NonNegativeReals)
# model.m = Var(model.M, model.N, within=NonNegativeReals)
# model.o = Var(model.M, within=NonNegativeReals)
# model.p = Var(model.O, within=NonNegativeReals)
# model.q = Var(model.O, within=NonNegativeReals)
# model.r = Var(model.O, within=NonNegativeReals)
# model.s = Var(model.M, within=NonNegativeReals)
#
...
# 创建拉格朗日松弛实例
# relaxed_constrs = ([model.time_constraint[i, j] for i in model.M for j in model.N]
...
for z in range(max_iter):
    lagrangian_relaxation.relax_constrs()
    lagrangian_relaxation.set_objective(origin_obj_rule)
    results = solver.solve(model, tee=True)
    # 计算子梯度
    lagrangian_relaxation.compute_subgrad()
    # 获取步长
    step_size = lagrangian_relaxation.get_step_size(z, max_iter)
    # 更新拉格朗日乘子
    ...
    # for i in model.M:
    #     for j in model.N:
    #         print(f'alpha[{i},{j}] = {model.alpha[i, j].value}')
    #
    # print('Beta values:')
    # for i in model.M:
    #     for j in model.N:
    #         print(f'beta[{i},{j}] = {model.beta[i, j].value}')
    #
    # print('Gamma values:')
    # for i in model.M:
    #     for j in model.N:
